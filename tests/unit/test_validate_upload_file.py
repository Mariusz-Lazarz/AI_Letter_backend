...
@pytest.mark.parametrize(
    ...
    [
        (b"%PDF valid content", "image/png", "application/pdf", "Only PDF files are allowed."),
        (b"a" * (6 * 1024 * 1024), "application/pdf", "application/pdf", "File size exceeds"),
        (b"%PDF-1.4 fake content", "application/pdf", "application/octet-stream", "not a valid PDF"),
    ]
)
@patch("helpers.validate_upload_file.magic.from_buffer")
async def test_validate_upload_file_errors(mock_magic, content, content_type, fake_mime, expected_error):
