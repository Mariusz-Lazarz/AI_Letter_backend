...
async def register(
    ...
    try:
        verification_token = sign_jwt(
            {"email": user_data.email}, expires_in=(ACCOUNT_CONFIRMATION_TOKEN)
        )
        ...
        background_tasks.add_task(
            email_sender.account_confirmation,
            to_email=user.email,
            verification_token=verification_token,
        )
...
@limiter.limit(
    RATE_LIMIT_VERIFY,
    key_func=lambda request: request.query_params.get("token", ""),
)
async def verify(request: Request, session: SessionDep, token: str):
    try:
        ...
        logger.log_exception(
            f"Verification failed for {user_email if 'user_email' in locals() else 'Unknown'}: {e}"
        )
...
async def login(
    ...
    try:
        ...
        refresh_token = sign_jwt(
            {"id": user.id, "email": user.email, "csrfToken": csrfToken},
            JWT_REFRESH_EXPIRE,
        )
...
async def forgot_password(
    ...
    return {
        "data": {
            "message": "If an account is associated with this email, you will receive an email shortly."
        }
    }
...
async def reset_password(
    request: Request, session: SessionDep, user_data: UserPasswordReset
):
    try:
